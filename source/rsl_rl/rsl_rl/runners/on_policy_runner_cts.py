...
class OnPolicyRunnerCTS:
    ...
    def learn(self, num_learning_iterations: int, init_at_random_ep_len: bool = False) -> None:
        ...
        for it in range(start_it, total_it):
            ...
            # Update policy
            loss_dict = self.alg.update(learning_iteration=it)
    ...
    def _construct_algorithm(self, obs: TensorDict) -> MoECTS:
        ...
        # Initialize the storage. Match MoECTS single-env play/eval: no teacher envs.
        teacher_env_ratio = self.alg_cfg["teacher_env_ratio"]
        if self.env.num_envs <= 1 or teacher_env_ratio <= 0.0:
            teacher_num_envs = 0
        else:
            teacher_num_envs = max(int(self.env.num_envs * teacher_env_ratio), 1)
            teacher_num_envs = min(teacher_num_envs, self.env.num_envs - 1) if teacher_env_ratio < 1.0 else self.env.num_envs
        storage = RolloutStorageCTS(
            "rl", self.env.num_envs, teacher_num_envs, self.cfg["num_steps_per_env"], obs, [self.env.num_actions], self.device
        )
