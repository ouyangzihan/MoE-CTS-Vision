...
class Go2RLGymCommand(CommandTerm):
    ...
    def __init__(self, cfg: Go2RLGymCommandCfg, env: Go2Env):
        ...
        self._start_command_ranges = {
            key: [float(self.command_ranges[key][0]), float(self.command_ranges[key][1])]
            for key in ("lin_vel_x", "lin_vel_y", "ang_vel_yaw")
        }
        self._max_command_ranges = self.cfg.command_range_max.to_dict()
    ...
    def _resample(self, env_ids: Sequence[int]):
        ...
        if len(env_ids) == 0:
            return
        # update command magnitude curriculum with train steps
    ...
    def _update_command_range_curriculum(self):
        ...
        if len(self.cfg.command_range_curriculum):
            for i in range(len(self.cfg.command_range_curriculum) - 1, -1, -1):
...
class Go2RLGymCommandCfg(CommandTermCfg):
    ...
    """Expand command ranges every N training iterations. Set to None/0 to disable."""

    @configclass
    class CommandRangeExpandCfg:
        lin_vel_x: float = 0.3
        """Expand lin_vel_x by this amount on each side per interval [m/s]."""
        lin_vel_y: float = 0.1
        """Expand lin_vel_y by this amount on each side per interval [m/s]."""
        ang_vel_yaw: float = 0.2
        """Expand ang_vel_yaw by this amount on each side per interval [rad/s]."""

    command_range_expand: CommandRangeExpandCfg = CommandRangeExpandCfg()
    """Per-axis expand magnitude applied to both sides each interval."""

    @configclass
    class CommandRangeMaxCfg:
        lin_vel_x: tuple[float, float] = [-2.0, 2.0]
        """Hard cap for lin_vel_x command range [m/s]."""
        lin_vel_y: tuple[float, float] = [-1.0, 1.0]
        """Hard cap for lin_vel_y command range [m/s]."""
        ang_vel_yaw: tuple[float, float] = [-2.0, 2.0]
        """Hard cap for ang_vel_yaw command range [rad/s]."""

    command_range_max: CommandRangeMaxCfg = CommandRangeMaxCfg()
    """Maximum command ranges. Gradual expansion never exceeds these."""

    command_range_curriculum: list[dict] = []
    """Legacy discrete milestones. Used only when command_range_expand_interval is disabled."""
