...
def track_lin_vel_xy_exp(
    ...
    if cmd.shape[-1] == 2:
        # Forward-only command ``(vx, yaw)``: track vx and keep lateral speed near 0.
        lin_vel_error = torch.square(cmd[:, 0] - asset.data.root_lin_vel_b[:, 0]) + torch.square(
...
def track_ang_vel_z_exp(
    ...
    # extract the used quantities (to enable type-hinting)
    asset: RigidObject = env.scene[asset_cfg.name]
    # Last dim is yaw for both ``(vx, vy, yaw)`` and ``(vx, yaw)`` layouts.
    yaw_cmd = env.command_manager.get_command(command_name)[:, -1]
    ang_vel_error = torch.square(yaw_cmd - asset.data.root_ang_vel_b[:, 2])
