...
class ActionManagerGo2W(ActionManagerGo2):
    """Go2W action manager with optional yaw-joint symmetry remapping.

    Policy actions are stored in canonical (positive-yaw) space for ``last_action``
    observations. When ``env.cfg.use_yaw_joint_symmetry`` is True and yaw < 0,
    actions are left-right mirrored before being applied to the robot.
    """
    ...
    def process_action(self, action: torch.Tensor):
        ...
        from robot_lab.tasks.go2w.mdp.yaw_joint_symmetry import apply_yaw_joint_symmetry_to_actions

        applied = apply_yaw_joint_symmetry_to_actions(self._env, self._action)

        idx = 0
        for term in self._terms.values():
            term_actions = applied[:, idx : idx + term.action_dim]
            term.process_actions(term_actions)
            idx += term.action_dim
