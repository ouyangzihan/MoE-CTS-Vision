...
class PlayJoy():
    def __init__(self, args):
        ...
        self.joy_cmd_velx = 0.9
        self.joy_cmd_vely = 0.9
        ...
        self.env_cfg.env.num_envs = min(self.env_cfg.env.num_envs, 2)

        self.env_cfg.terrain.max_init_terrain_level = 5  # starting curriculum state

        self.env_cfg.terrain.num_rows = self.env_cfg.terrain.max_init_terrain_level + 1
        self.env_cfg.terrain.num_cols = 2

        self.env_cfg.terrain.curriculum = True
        ...
        self.env_cfg.terrain.border_size = 5  # [m]
    ...
    def play(self):
        ...
        while not rospy.is_shutdown():
            ...
            images_clean = np.round(images_clean, decimals=2)
            # print('images_clean', images_clean)
            # print()

            # ************ for debug ************
            obs_float = obs[0].cpu().numpy().squeeze().astype(np.float32)
    ...
    def joy_callback(self, joy_msg):
        self.joy_cmd_velx = joy_msg.axes[4] * 1
        # print(joy_msg.axes[4], self.joy_cmd_velx )
        self.joy_cmd_vely = joy_msg.axes[3] * 1.0

        # print(joy_msg.axes[3], self.joy_cmd_vely)
        # self.joy_cmd_heading = joy_msg.axes[0] * 1
        self.joy_cmd_ang_vel = joy_msg.axes[0] * 1
