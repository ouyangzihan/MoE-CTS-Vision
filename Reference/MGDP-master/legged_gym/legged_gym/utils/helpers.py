...
def update_cfg_from_args(env_cfg, cfg_train, args):
    ...
    if env_cfg is not None:
        ...
        if getattr(env_cfg, 'camera', None) is not None:
            ...
            if getattr(args, 'load_world_model_policy', None) is False:
...
def get_args():
    custom_parameters = [
        ...
        {"name": "--render_device", "type": int,
         "help": "render image device."},

        {"name": "--num_envs", "type": int,
         "help": "Number of environments to create. Overrides config file if provided."},
        {"name": "--graphics_device_num", "type": int, "default": None,
         "help": "Graphics/GPU device id (e.g. 0 for cuda:0). Overrides config env.graphics_device_num if provided."},
