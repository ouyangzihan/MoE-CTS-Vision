...
class TaskRegistry():
    ...
    def make_alg_runner(self, env, name=None, args=None, train_cfg=None, log_root="default"):
        ...
        print("Setting args seed: {}".format(args.seed))
        print("Setting train_cfg seed: {}".format(train_cfg.seed))
        ...
        if not train_cfg.runner.resume:
            ...
            if log_dir is not None:
                last_ckpt_path = os.path.join(
                    log_dir,
                    'stage2_nn' if args.algo == "ProprioAdapt" else 'stage1_nn',
                    'last.pt'
                )
        ...
        if args.resume_name is not None:
            if log_root is not None:
                ...
                log_dir1 = args.resume_name

        save_to_log_dir = (log_dir is not None) and (not resume or (log_dir != log_dir1))
        if save_to_log_dir:
            ...
            copyfile(os.path.join(LEGGED_GYM_ROOT_DIR, 'legged_gym', 'envs', 'baseline', 'legged_robot_terrains.py'), log_dir + '/legged_robot_terrains.py')
            copyfile(os.path.join(LEGGED_GYM_ROOT_DIR, 'legged_gym', 'envs', 'base', 'legged_robot.py'), log_dir + '/legged_robot.py')
            copyfile(os.path.join(LEGGED_GYM_ROOT_DIR, 'legged_gym', 'envs', 'baseline', 'legged_robot_rewards.py'), log_dir + '/legged_robot_rewards.py')
        ...
        if resume and log_dir is not None and log_dir != log_dir1 and log_dir1 is not None:
            ...
            copyfile(os.path.join(LEGGED_GYM_ROOT_DIR, 'legged_gym', 'envs', 'baseline', 'legged_robot_terrains.py'), log_dir1 + '/legged_robot_terrains.py')
            copyfile(os.path.join(LEGGED_GYM_ROOT_DIR, 'legged_gym', 'envs', 'base', 'legged_robot.py'), log_dir1 + '/legged_robot.py')
            copyfile(os.path.join(LEGGED_GYM_ROOT_DIR, 'legged_gym', 'envs', 'baseline', 'legged_robot_rewards.py'), log_dir1 + '/legged_robot_rewards.py')
            copyfile(os.path.join(LEGGED_GYM_ROOT_DIR, 'legged_gym', 'envs', 'random_dog', 'utils', 'sensor_config.py'), log_dir1 + '/sensor_config.py')
            copyfile(os.path.join(LEGGED_GYM_ROOT_DIR, 'legged_gym', 'envs', 'random_dog', 'utils', 'net_work.py'), log_dir1 + '/net_work.py')
        ...
        if resume:
            ...
            resume_path = get_load_path(os.path.join(log_dir1, str(model_dir)), checkpoint=args.checkpoint_model)

            print(f"Loading model from: {log_dir1}, model_dir={model_dir}, resume_path={resume_path}")
