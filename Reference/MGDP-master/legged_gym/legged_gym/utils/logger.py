...
class Logger:
    ...
    def _plot(self):
        ...
        # plot Base vel x
        a = axs[0, 0]
        if log["pre_vel_x"]: a.plot( log["pre_vel_x"], label='pre_vel_x')
        if log["real_vel_x"]: a.plot( log["real_vel_x"], label='real_vel_x')
        if log["command_x"]: a.plot( log["command_x"], label='command_x')
        a.set(xlabel='time [s]', ylabel='compare x vel [m/s]')
        a.legend()

        # plot Base vel y
        a = axs[0, 1]
        if log["pre_vel_y"]: a.plot( log["pre_vel_y"], label='pre_vel_y')
        if log["real_vel_y"]: a.plot( log["real_vel_y"], label='real_vel_y')
        if log["command_y"]: a.plot( log["command_y"], label='command_y')
        a.set(xlabel='time [s]', ylabel='compare y vel [m/s]')
        a.legend()

        # plot Base vel z
        a = axs[1, 0]
        if log["pre_vel_z"]: a.plot(log["pre_vel_z"], label='pre_vel_z')
        if log["real_vel_z"]: a.plot( log["real_vel_z"], label='real_vel_z')
        a.set(xlabel='time [s]', ylabel='base lin vel [m/s]')
        a.legend()

        # plot Base vel z
        a = axs[1, 1]
        if log["real_yaw"]: a.plot(log["real_yaw"], label='real_yaw')
        if log["command_yaw"]: a.plot(log["command_yaw"], label='command_yaw')
        a.set(xlabel='time [s]', ylabel='compare y vel [m/s]')
        a.legend()
    ...
    def _plot_base(self):
        ...
        if log["pre_vel_x"]: a.plot(time, log["pre_vel_x"], label='pre_vel_x')
